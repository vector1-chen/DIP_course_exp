...
int main(int argc, char *argv[])
{
        ...
        Mat raw_circle = imread("/home/eaibot/dip_ws/src/exp3/data/circle.png");
        while (waitKey(10))
        {
                /***************读取图像***************/
                ...
                Mat gray_line = raw_line.clone(),gray_circle = raw_circle.clone();
                cvtColor(raw_line, gray_line, COLOR_BGR2GRAY);
                cvtColor(raw_circle, gray_circle, COLOR_BGR2GRAY);

                /****************调用边缘检测函数****************/
                Mat gray_line_e = Mat::zeros(gray_line.size(), CV_8UC1);
                EdgeDetector(gray_line, gray_line_e);
                imshow("EdgeDetector_line", gray_line_e);
                Mat gray_circle_e = Mat::zeros(gray_circle.size(), CV_8UC1);
                EdgeDetector(gray_circle, gray_circle_e);
                imshow("EdgeDetector_circle", gray_circle_e);

                /***************调用霍夫线变换***************/
                Mat gray_line_l = Mat::zeros(gray_line.size(), CV_8UC1);
                Mat raw_line_edit = raw_line.clone();
                HoughLines(gray_line, raw_line_edit);
                imshow("HoughLines", raw_line_edit);
...
void HoughCircles(Mat input_gray, Mat output){
    ...
    for (int y = 0; y < center_votes.rows; y++)
    {
        for (int x = 0; x < center_votes.cols; x++)
        {
            ...
            {
                // 估计半径
                vector<int> radius_votes(max_radius - min_radius, 0);
                for (int yy = 0; yy < height; yy++)
                {
                    for (int xx = 0; xx < width; xx++)
                    {
                        ...
                        {
                            int r = cvRound(sqrt((xx - x) * (xx - x) + (yy - y) * (yy - y)));
                            if (r >= min_radius && r <= max_radius)
                            {
                                radius_votes[r - min_radius]++;
                            }
                        }
                    }
                }
                ...
                {
                    if (radius_votes[r - min_radius] > max_votes)
                    {
                        max_votes = radius_votes[r - min_radius];
                        best_radius = r;
                    }
                }
                circle(output, Point(x, y), best_radius, Scalar(0,0,255), 1);
            }
        }
    }
}
